...
class cubes:
    ...
    def __init__(self, initial_cubes, size):
        self.min_size = -1
        self.size = size + 1
        self.d = [(x, y, z, w) for x in range(-1, 2) for y in range(-1, 2) for z in range(-1, 2) for w in range(-1, 2)]
        self.d.remove((0, 0, 0, 0))
        self.grid = defaultdict(lambda: defaultdict(lambda: defaultdict(lambda: defaultdict(lambda: cube()))))
        self.active = 0
        for c in initial_cubes:
            x, y, z, w, active = c
            self.grid[x][y][z][w].active = active
            self.active += active
    ...
    def assign_all_neighbours(self):
        for x in range(self.min_size, self.size):
            for y in range(self.min_size, self.size):
                for z in range(self.min_size, self.size):
                    for w in range(self.min_size, self.size):
                        self.assign_neighbours(x, y, z, w)

    def assign_neighbours(self, x, y, z, w):
        c = self.grid[x][y][z][w]
        c.neighbours = []
        for dx, dy, dz, dw in self.d:
            c.add_neighbour(self.grid[x + dx][y + dy][z + dz][w + dw])
...
def parse():
    ...
    for x in range(len(layout)):
        for y in range(len(layout[x])):
            active_cubes.append((x, y, 0, 0, layout[x][y]))
